    ...
    if (tc->use_legacy_pitch_filter) {
        ...
        if (tc->def->flags & TRAKTOR_MK2) {
            pitch_kalman_init(&tc->pitch_kalman_filter, tc->dt,
                              ...
                              KALMAN_COEFFS(0.018, 1e-5), /* scratch mode */
                              15, /* threshold */
                              false);
        } else {
            pitch_kalman_init(&tc->pitch_kalman_filter, tc->dt,
...
void timecoder_clear(struct timecoder *tc)
{
    ...
    if (tc->def->flags & TRAKTOR_MK2) {
        delayline_init(&tc->primary.delayline);
        delayline_init(&tc->secondary.delayline);
    ...
    signed int secondary)
{
    ...
    delayline_push(&tc->primary.delayline_deriv, tc->primary.deriv_scaled);
    delayline_push(&tc->secondary.delayline_deriv, tc->secondary.deriv_scaled);
}
			   ...
			   signed int primary, signed int secondary)
{
    ...
    if (tc->dB > -40.0) {
        tc->dphi =
            phase_difference(delayline_at(&tc->primary.delayline_deriv, 0),
                             delayline_at(&tc->secondary.delayline_deriv, 0),
                             delayline_at(&tc->primary.delayline_deriv, 1),
                             delayline_at(&tc->secondary.delayline_deriv, 1));
